class User < ApplicationRecord
  ...
  scope :maint_list, -> { where(active: true, position_id: Position.where(department: 'Maintenance').ids)
                            .or(where(active: true, position_id: Position.where(name: ["HR Manager", "Director", "Business Director", "Marketing Manager", "Payroll Manager", "AR Manager", "AP Manager", "Training Manager"]).ids)).order(first_name: :asc) }
